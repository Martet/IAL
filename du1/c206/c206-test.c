 ...
 ******************************************************************************/
static void checkInterconnectedness(DLList TL) {
    if(TL.firstElement == NULL || TL.lastElement == NULL) {
        if(TL.firstElement != NULL || TL.lastElement != NULL) {
            printf("one of {firstElement, lastElement} is NULL and the other one isn't\n");
        }
    }
    int foundActive = 1;
    if(TL.activeElement != NULL) {
        foundActive = 0;
    }

    DLLElementPtr curr = TL.firstElement;
    DLLElementPtr tmp = NULL;
    while(curr != NULL) {
        if(curr == TL.activeElement) {
            foundActive = 1;
        }
        tmp = curr;
        if(curr->nextElement != NULL) {
            if(curr->nextElement->previousElement != curr) {
                printf("next element doesn't link back to current one\n");
            }
        }
        curr = curr->nextElement;
    }
    if(tmp != TL.lastElement) {
        printf("last reached element != list->lastElement\n");
    }
    if(!foundActive) {
        printf("active element is set, but is not reachable\n");
    }
}
